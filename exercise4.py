...
class EscapeRoomCommandHandler:
    ...
    def _cmd_open(self, open_args):
        ...
        object = self.room["container"].get(open_args[0], None)# !!!change the argument from -1 to 0 avoiding AttributeError
        
        #success_result = "You open the {}.".format(object.name)
        # !!! rewrite this line, don't call object.name
        success_result = "You open it! "
        open_result = (
    ...
    def _cmd_hit(self, hit_args):
        if len(hit_args) == 0:
            ...
            success_hit_result = "You hit it!"
            if len(hit_args) == 1:
                hit_result = (
                    ((not object or not object["visible"]) and "There is no {}".format(hit_args[0])) or 
                    ((not object["hittable"])              and "You can't hit that") or
                    ( object["hittable"]                   and "Hit with what?"))
            else:
                hitter = self.player["container"].get(hit_args[-1], None)
                hit_result = (
                    ((not object or not object["visible"]) and "There is no {}".format(hit_args[0])) or
                    ((not object["hittable"])              and "You can't hit that") or
                    ((object["hittable"] and not hitter)   and "You don't have a {}".format(hit_args[-1])) or
                    ((object["hittable"] and hitter not in object["hitter"]) and "You can't hit {} with that".format(object.name)) or
                                                            success_hit_result)

            if hit_result == success_hit_result:
                object["visible"] = False
                self._run_triggers(object, "hit")
...
def create_door_description(door):
    ...
    if door["locked"]:
    # !!!change the description of door a little bit
        return description+" The door is locked." 
    else:
        return description+" The door seems able to be opened"
...
class EscapeRoomGame:
    ...
    def create_game(self, cheat=False):
        ...
        # !!!add key here
        key = EscapeRoomObject("key",visible=False,gettable=True)
        # !!!add flying key here
        flyingkey = EscapeRoomObject("flyingkey",visible=True,gettable=False,hittable=True,hitted=False,hitter=[hammer])
        ...
        room["container"]  = create_container_contents(player, door, clock, mirror, hairpin, chest, key, flyingkey)
        ...
        # !!! add a trigger for flying key to make key visible
        flyingkey.triggers.append(lambda obj, cmd, *args: (cmd == "hit") and key.__setitem__("visible",True))
        flyingkey.triggers.append(lambda obj, cmd, *args: (cmd == "hit") and flyingkey.__setitem__("hitted",True))
...
def main(args):
    async def move_flyingkey(game):
        positions = ["celling","wall","ground"]
        i = random.choice(range(3))
        while True:
            position = positions[i]
            print("!!!!!!!flyingkey moved")
            i = i+1 if i < 2 else 0
            if positions[i] != "ground":
                game.room["container"]["flyingkey"]["hittable"] = False
            else:
                game.room["container"]["flyingkey"]["hittable"] = True
    ...
    async def running_game(game):
        i = 0
        while game.status == "playing":
            print(">> "+command_list[i])
            game.command(command_list[i])
            if command_list[i] == "hit flyingkey with hammer" and not game.room["container"]["flyingkey"]["hitted"]:
                i -= 1
            i += 1
    ...
    try:
        ...
        asyncio.ensure_future(running_game(game))
        asyncio.ensure_future(move_flyingkey(game))
