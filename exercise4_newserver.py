...
def move_flyingkey(game,flyingkey):
        locations = ["ceiling","floor","wall"]
        locations.remove(game.flyingkey["location"])
        random.shuffle(locations)
        next_location = locations.pop(0)
        old_location = game.flyingkey["location"]
        game.flyingkey["location"] = next_location
        game.flyingkey["description"] = create_flyingkey_description(game.flyingkey)
        game.flyingkey["short_description"] = create_flyingkey_short_description(game.flyingkey)
        game.flyingkey["hittable"] = next_location == "wall"
        game.output("The {} flies from the {} to the {}".format(game.flyingkey.name, old_location, next_location))
        for event in game.room.do_trigger("_post_command_"):
            game.output(event)

async def flyingkey_agent(game,flyingkey):
        # this is the part you, the student, fills in.
        # you will probably need to use:
        #  - asyncio.sleep (I recommend 5 seconds)
        #  - self.status, you'll need to stop when no longer playing
        ...
        while game.status == "playing":
            game.move_flyingkey(game,game.flyingkey)
            await asyncio.sleep(5)
# server class
...
async def main(args):
    ...
    game_server = EchoServer()
    coro = loop.create_server(game_server,'192.168.200.116',2344)

    await asyncio.wait(flyingkey_agent(game_server.game,game_server.game.flyingkey))
